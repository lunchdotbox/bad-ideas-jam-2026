...
void tickIntroCutscene(IntroCutscene* intro, TextFont* font, SoundsGame* sounds, u64 ct) {
    switch (intro->state) {
        ...
        case INTRO_STATE_DISPLAY_DANIEL_TEXT:
            ...
            addTextRectangle(font, COLOR_BLACK, (vec2){-2.0f, -2.0f}, (vec2){4.0f, 4.0f});
            addTextCentered(font, (vec2){-2.0f + easeOutElastic(easeInQuad((float)(ct - intro->start_time) / stoms(1.0))) * 2.0f, -0.25f}, (vec2){0.4f, 0.4f}, COLOR_BURLYWOOD, "daniel");
            break;
        case INTRO_STATE_DISPLAY_DRIVE_TEXT:
            ...
            addTextCentered(font, (vec2){2.0f - easeOutElastic(easeInQuad((float)(ct - intro->start_time) / stoms(1.0))) * 2.0f, 0.25f}, (vec2){0.4f, 0.4f}, COLOR_RED, "drive");
            break;
