...
/*ELC_INLINE*/ void updateFractionMomentum(GasSystem* gas, dvec2 dir, double area, double fract_mass, double fract_vol, double dt) {
    double fract_vel = CLAMP((fract_vol / area) / dt, 0.0, gasSystemC(*gas));

    gas->momentum[0] += fract_vel * dir[0] * fract_mass;
    gas->momentum[1] += fract_vel * dir[1] * fract_mass;
}

/*ELC_INLINE*/ void conserveEnergy(GasSystem* gas, dvec2 initial_momentum, double inv_mass) {
    gas->total_speed -= 0.5 * mass(*gas) * (pow2(gas->momentum[0] * inv_mass) - pow2(initial_momentum[0] * inv_mass));
    gas->total_speed -= 0.5 * mass(*gas) * (pow2(gas->momentum[1] * inv_mass) - pow2(initial_momentum[1] * inv_mass));
}
...
/*ELC_INLINE*/ double flowSourceToSink(GasSystem* src, GasSystem* dst, dvec2 dir, double flow_dir, double area_src, double area_dst, double p_src, double p_dst, double k_flow, double dt) {
    ...
    double im_src = 1.0 / mass(*src);
    double im_dst = 1.0 / mass(*dst);
    ...
    if (area_dst != 0.0) updateFractionMomentum(dst, dir, area_dst, fract_mass, fract_vol, dt);
    if (area_src != 0.0 && mass(*src) != 0.0) updateFractionMomentum(src, dir, area_src, fract_mass, fract_vol, dt);

    if (mass(*src) != 0.0) conserveEnergy(src, initial_momentum_src, im_src);
    if (mass(*dst) > 0.0) conserveEnergy(dst, initial_momentum_dst, im_dst);
